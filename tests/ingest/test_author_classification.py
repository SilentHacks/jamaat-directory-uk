...
def test_model_no_timetable_is_terminal_without_escalation(engine, tmp_path):
    ...
    out = author_mosque(engine, "m1", harness=harness, candidate_root=tmp_path,
                        models=("cheap", "strong"), today=TODAY, horizon_days=5,
...
def test_discovery_under_construction_is_no_timetable(engine, tmp_path):
    ...
    out = discover_mosque(engine, "uc", fetcher=fetcher, client=_live_client(),
                          candidate_root=tmp_path, today=TODAY, horizon_days=20)

    assert out.outcome == "no_timetable"
    assert not (tmp_path / "uc.json").exists()  # no AI hand-off bundle written
    with session_scope(engine) as s:
...
def test_discovery_restaurant_is_wrong_site(engine, tmp_path):
    ...
    out = discover_mosque(engine, "rest", fetcher=fetcher, client=_live_client(),
                          candidate_root=tmp_path, today=TODAY, horizon_days=20)
...
def test_discovery_keyword_links_still_flow_to_candidate(engine, tmp_path):
    ...
    out = discover_mosque(engine, "amb", fetcher=fetcher, client=_live_client(),
                          candidate_root=tmp_path, today=TODAY, horizon_days=20)
...
def test_discovery_terminal_does_not_overwrite_existing_config(engine, tmp_path):
    ...
    out = discover_mosque(engine, "keep", fetcher=fetcher, client=_live_client(),
                          candidate_root=tmp_path, today=TODAY, horizon_days=20)
...
def test_discovery_terminal_bundle_carries_evidence(engine, tmp_path):
    ...
    discover_mosque(engine, "amb2", fetcher=fetcher, client=_live_client(),
                    candidate_root=tmp_path, today=TODAY, horizon_days=20)
